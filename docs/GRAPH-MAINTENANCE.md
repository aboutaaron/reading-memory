...
The retest found zero eligible model relationships and 269 heuristic relationships. It also marked all existing analyses stale. These are observations about stored data and configured policy, not evidence that the current analysis model is poor. Production was confirmed to use `openai/gpt-5.6-luna` with embeddings disabled. Keep that analysis model for the first audit.

## Embeddings and the analysis model are separate choices

The retest supports selective `hybrid` / `any` recall, with a modest held-out gain; it does not justify switching all natural-language queries to strict matching. To roll out that option after validation, set `READING_API_EMBEDDING_MODEL=openai/text-embedding-3-small` in the intended service environment, retain its existing analysis model, restart that service and confirm `/health.embeddings` reports the expected model and available index. Use the existing private `OPENAI_API_KEY`; never print it. See [hybrid configuration and backfill](HYBRID-RETRIEVAL.md).
