...
Declare whether the frozen labels describe required edges or acceptable alternatives. If they enumerate alternatives, the three-edge output cap means exhaustive typed-edge recall is not an appropriate headline, and `missing_expected_edges` does not automatically mean useful evidence was omitted. Likewise, an outside-label edge can be legitimate: keep the original automatic agreement scores and separately review its semantic usefulness rather than silently rewriting labels. Report target coverage and useful accepted edges with their denominators, cohorts, and reviewer uncertainty.
