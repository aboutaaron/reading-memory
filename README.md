...
Use the [evidence-quality retest](docs/OPENCLAW-EVIDENCE-RETEST.md) for the latest extraction, source-family diversity, and passage-reference checks. The [analyzer comparison](docs/ANALYZER-COMPARISON.md) runs two explicitly selected models against frozen inputs; it defaults to a dry run and does not change the production model or corpus. See the implementation guides for [article extraction](docs/ARTICLE-EXTRACTION.md), [source families](docs/SOURCE-FAMILIES.md), and [passage evidence](docs/PASSAGE-EVIDENCE.md).

Use the [answer/source evaluation](docs/ANSWER-SOURCE-EVALUATION.md) to check requested-source alignment and provenance claims separately from exact quotations. Synthetic controls and manual semantic review serve different purposes; genuine quotes alone do not establish a correct answer.
