...
After using a stored source in an answer, record a `cited` event through `/brief-events` with `included_bool: true`, a meaningful rationale, and a stable answer identifier in `source_context`. Retrieval alone is not a citation. A citation neither consumes a brief appearance nor changes a pending resurfacing schedule.
