...
URL/PDF captures extract available title, author, publisher, and publication date. An explicit `source.title` takes precedence. Full normalized text determines content identity before the stored analysis text is truncated; `truncated` is returned on ingest and item reads. `GET /items/:id` also returns the retained extracted text, annotation history, analysis rationale, and relationship evidence.
