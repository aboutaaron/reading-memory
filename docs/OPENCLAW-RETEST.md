...
Review returned sources outside the initial expected set before labelling them irrelevant. Pool and deduplicate candidates across arms, hide the originating mode/policy, scores and ranks, and shuffle review order so relevance judgments are blinded to the retrieval method. Record relevant, useful background, irrelevant and uncertain; add justified newly discovered relevant IDs to a reviewed label set and report both original-label and reviewed-label metrics. Do not silently tune questions or labels after seeing one arm's output.
...
Provide the tested revision/runtime; baseline build/test/eval summaries; corpus/embedding/edge coverage; provider calls, timing and known or unknown cost; paired metric tables by arm and cohort; five concrete examples spanning improvements, regressions and absent/near-miss handling; graph relation-audit results; OpenClaw answer-support results; operational pass/fail/not-run table; and minimal sanitized reproduction steps for each bug. State whether OpenClaw should keep broad retrieval, explicitly request `all`, use graph selectively for relationship questions, or wait for further fixes. Recommend strict lexical policy only when its reviewed reduction in irrelevant candidates justifies the observed positive recall losses. Recommend graph for relationship questions only when audited useful additions outweigh useful direct hits displaced and do not increase unsupported answers. Treat operational failures as blockers and small or uncertain quality differences as inconclusive. Keep retrieval modes opt-in unless the evidence supports a separate rollout decision.
