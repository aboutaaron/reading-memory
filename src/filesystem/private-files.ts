import { constants, closeSync, fchmodSync, fstatSync, lstatSync, mkdirSync, openSync, realpathSync, type Stats } from 'node:fs';
...
export function privateFile(path: string, { create = false, optional = false }: { create?: boolean; optional?: boolean } = {}): boolean {
...
export function privateDatabasePath(path: string, { create = false }: { create?: boolean } = {}): string {
  const absolute = resolve(path);
...
export function assertAbsent(path: string): void {
